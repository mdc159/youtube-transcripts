...
A two-phase CLI that turns a YouTube URL or local video into a citation-grounded multimodal study note. Phase 1 (`extract.py`) downloads the source, runs a 4-tier transcript chain, extracts and OCRs frames, classifies what's on screen, and writes an idempotent artifact tree. Phase 2 (`distill.py`) reads those artifacts, builds an enriched + multimodal payload, calls a vision-capable LLM, and validates every citation in the result. `run.py` chains both for one-shot use.
...
| `frame_select.py` | Perceptual-hash scene detection + anchored even-spacing + token-budget cap |
